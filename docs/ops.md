...
1. **The repository serves Connect.** This service calls it with Connect
   clients, so a repository older than v1.9.0 answers every call
...
2. **Postgres reachable and migrated.** Migrations are never run by the
   ...
   first query.
3. **The password key is set and 32 bytes.** Startup fails on a malformed key
...
4. **The repository is reachable.** The schema loader fetches revisor schemas
   at startup and startup fails without them.
5. **At least one cluster is registered and one index set exists.** With
...
6. **An index set is active.** Search against no active set fails; the
   readiness check does not.

Out of order, the failures are all at startup except step 5, which is the one
